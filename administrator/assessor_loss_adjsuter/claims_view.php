...
if(isset($_GET['l']) && is_numeric($_GET['l']) && ( $_GET['s']==0 || is_numeric($_GET['s']) ) && isset($_GET['re']) && !empty($_GET['re']) && isset($_GET['e']) && !empty($_GET['e']) && isset($_GET['f']) && !empty($_GET['f'])  && !empty($_GET['_id']) && isset($_GET['_id']) )
{
        ...
        $header_array= array('Cookie:'.$_SESSION['cookie'],'Authorization:'.api_key_is(),'Origin:/assessor_loss_adjsuter/claims_view.php');
                  ...
                  $table='<table class="table table-bordered table-hover table-responsive">
                        ...
                        $count=0;//set count
                                                                ...
                                                                {//start of foreach $message_is as $value
                                                                      ...
                                                                      $table_assigned_assessors=$table_assigned_assessors.'<tr class="'.$row_color.'" id="row_data">
                                                                                                    <td>'.($count+1).'</td>  
                                                                                                                    ...
                                                                                                                    <td id="view_add_report_td'.$count.'" ><a href="'.$view_assessor_reports.'" title="View and/or add report for claim number '.$claim_number.'" class="tbutton">View/create report</a></td>
                                    ...
                                    $count=0;//set count
                                                ...
                                                //make add add repair garage link 
                                                    $add_repair_garage_link="claims_assign_repair_garage.php?l=".$limit."&s=".$skip."&re=".$rows_every."&e=".$email_address."&f=".$full_names."&_id=".$_id.'&c_n='.$claim_number;//for form submission

                                                    //append add repair garage
                                                    $table.='<h4>Assigned repair garages</h4><br>'.$table_assigned_repair_garages.'<a href="'.$add_repair_garage_link.'" title="Assign repair garage to '.$claim_number.'" class="btn btn-primary m-t-15 waves-effect">Assign</a><br><br>';

...
if(isset($_GET['l']) && is_numeric($_GET['l']) && ( $_GET['s']==0 || is_numeric($_GET['s']) ) && isset($_GET['re'])  && !empty($_GET['re']) && isset($_GET['e']) && !empty($_GET['e']) && isset($_GET['f']) && !empty($_GET['f'])  && !empty($_GET['_id']) && isset($_GET['_id']) && isset($_GET['c_n']) && !empty($_GET['c_n']) )
{
        ...
        $full_link="claims_assign_repair_garage.php?l=".$limit."&s=".$skip."&re=".$rows_every."&e=".$email_address."&f=".$full_names."&_id=".$_id."&c_n=".$claim_number;//for form submission
        $return_link="claims_view.php?l=".$limit."&s=".$skip."&re=".$rows_every."&e=".$email_address."&f=".$full_names."&_id=".$_id;//for form submission
        
        //get dropdown list
        $garages_list=fetch_garages_drop_down_list('/assessor_loss_adjsuter/claims_assign_repair_garage.php');
       
            
                  //submit
                    if(isset($_POST['repair_garages_list']) && !empty($_POST['repair_garages_list'])  )
                    {  
                       
                        $repair_garages_list=trim($_POST['repair_garages_list']);

                       
                            $url_is=the_api_authentication_api_url_is()."denkimAPILogic/MainPackages.AssessorLossAdjusterAssignClaimToRepairGarage";

                            $myvars='session_key='.$_SESSION['session_key'].'&claim_id='.$_id.'&repair_garage_email='.$repair_garages_list;

                             $header_array= array('Cookie:'.$_SESSION['cookie'],'Authorization:'.api_key_is(),'Origin:/assessor_loss_adjsuter/claims_assign_repair_garage.php');
                            ...
                            {
                                     //get garage phone number
                                    $garage_info=get_specific_repair_garage_details($_SESSION['session_key'],$_SESSION['cookie'],$repair_garages_list,'/assessor_loss_adjsuter/claims_assign_assessor.php');
                                    ...
                                    send_sms_message($_SESSION['session_key'],$_SESSION['cookie'],$message_send,$garage_info['phone_number'],'/assessor_loss_adjsuter/claims_assign_assessor.php');
...
<body class="theme-orange">
    ...
    <section class="content">
        <div class="container-fluid">
		   ...
		   <h4>Assign Repair garage to claim number: <?php echo $claim_number;?></h4>    
          <?php echo $message;?><br>
            
          <form action="<?php echo $full_link;?>" method="POST">
             <?php echo $garages_list;?></br>

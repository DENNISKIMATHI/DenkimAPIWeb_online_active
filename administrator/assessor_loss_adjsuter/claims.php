...
if(isset($_GET['l']) && is_numeric($_GET['l']) && ( $_GET['s']==0 || is_numeric($_GET['s']) ) && isset($_GET['re']) && !empty($_GET['re']) && isset($_GET['e']) && !empty($_GET['e']) && isset($_GET['f']) && !empty($_GET['f']))
{
        ...
        $header_array= array('Cookie:'.$_SESSION['cookie'],'Authorization:'.api_key_is(),'Origin:/assessor_loss_adjsuter/claims.php');
        ...
        if($check_is==true)//if check is true
        {
            ...
            //get email details
            $details=fetch_personal_details(2,$_SESSION['session_key'],$_SESSION['cookie'],'/assessor_loss_adjsuter/claims.php');
            //fetch claims assigned to this assessor only
            $message_is=filter_claims_for_assessor_loss_adjuster($message_is,'/assessor_loss_adjsuter/claims.php',$details['email_address']);

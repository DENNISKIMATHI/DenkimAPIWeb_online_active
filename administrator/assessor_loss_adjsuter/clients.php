...
if( isset($_GET['l']) && is_numeric($_GET['l']) && ( $_GET['s']==0 || is_numeric($_GET['s']) ) &&  isset($_GET['sc']) && !empty($_GET['sc']) && isset($_GET['so']) && !empty($_GET['so']) && isset($_GET['re']) && !empty($_GET['re']))
{
        ...
        $header_array= array('Cookie:'.$_SESSION['cookie'],'Authorization:'.api_key_is(),'Origin:/assessor_loss_adjsuter/clients.php');
        ...
        if($check_is==true)//if check is true
        {
            ...
            $details=fetch_personal_details(2,$_SESSION['session_key'],$_SESSION['cookie'],'/assessor_loss_adjsuter/clients.php');
            $message_is=filter_clients_for_assessor_loss_adjuster($message_is,$details,'/assessor_loss_adjsuter/clients.php');
                ...
                $message='<span id="bad_upload_message">'.$message_is.'</span>';
                    ...
                    {  
                        ...
                        if($antispam==$_SESSION['spam'])
                        {
                            ...
                            $header_array= array('Authorization:'.api_key_is(),'Origin:/assessor_loss_adjsuter/clients.php');

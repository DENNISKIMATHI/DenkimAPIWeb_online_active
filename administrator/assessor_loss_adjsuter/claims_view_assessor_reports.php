        ...
        )
{
        ...
        //get email details
        $details=fetch_personal_details(2,$_SESSION['session_key'],$_SESSION['cookie'],'/assessor_loss_adjsuter/claims_view_assessor_reports.php');
        
        if($details['email_address']==$assessor_email)//can make report give link to report
        {
            $claims_create_garage_report_link="claims_create_assessor_report.php?l=".$limit."&s=".$skip."&re=".$rows_every."&e=".$email_address."&f=".$full_names."&_id=".$_id.'&c_n='.$claim_number."&a_e=".$assessor_email."&c_h_e=".$claim_handler_email."&t_s=".$time_stamp."&a_f_n=".$assessor_full_names."&a_p_n=".$assessor_phone_number."&c_h_f_n=".$claim_handler_full_names.'&c_h_p_n='.$claim_handler_phone_number;//for form submission
            $create_report_message='<a href="'.$claims_create_garage_report_link.'" title="Create report for claim number '.$claim_number.'" class="tbutton">Create report</a>';
        }   
        else
        {
            $create_report_message="You cannot create a report because this claim is not assigned to you!";
        }
        ...
        $header_array= array('Cookie:'.$_SESSION['cookie'],'Authorization:'.api_key_is(),'Origin:/assessor_loss_adjsuter/claims_view_assessor_reports.php');

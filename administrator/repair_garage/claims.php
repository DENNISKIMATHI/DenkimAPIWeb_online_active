...
if(isset($_GET['l']) && is_numeric($_GET['l']) && ( $_GET['s']==0 || is_numeric($_GET['s']) ) && isset($_GET['re']) && !empty($_GET['re']) && isset($_GET['e']) && !empty($_GET['e']) && isset($_GET['f']) && !empty($_GET['f']))
{
        ...
        $header_array= array('Cookie:'.$_SESSION['cookie'],'Authorization:'.api_key_is(),'Origin:/repair_garage/claims.php');
        ...
        if($check_is==true)//if check is true
        {
            ...
            //get email details
            $details=fetch_personal_details(2,$_SESSION['session_key'],$_SESSION['cookie'],'/repair_garage/claims.php');
            //fetch claims assigned to this garage only
            $message_is=filter_claims_for_repair_garage($message_is,'/repair_garage/claims.php',$details['email_address']);
            ...
            {//start of foreach $message_is as $value
                  ...
                  $table=$table.'<tr class="'.$row_color.'" id="row_data">
                                                <td>'.($count+1).'</td>  
                                                                ...
                                                                <td id="view_td'.$count.'" ><a href="'.$view_link.'&_id='.$_id.'" title="View and edit '.$claim_number.'" class="tbutton">View/create report</a></td>  

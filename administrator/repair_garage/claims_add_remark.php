...
if(isset($_GET['l']) && is_numeric($_GET['l']) && ( $_GET['s']==0 || is_numeric($_GET['s']) ) && isset($_GET['re'])  && !empty($_GET['re']) && isset($_GET['e']) && !empty($_GET['e']) && isset($_GET['f']) && !empty($_GET['f'])  && !empty($_GET['_id']) && isset($_GET['_id']) && isset($_GET['c_n']) && !empty($_GET['c_n']) )
{
        ...
        $full_link="claims_add_remark.php?l=".$limit."&s=".$skip."&re=".$rows_every."&e=".$email_address."&f=".$full_names."&_id=".$_id."&c_n=".$claim_number;//for form submission
        $return_link="claims_view.php?l=".$limit."&s=".$skip."&re=".$rows_every."&e=".$email_address."&f=".$full_names."&_id=".$_id;//for form submission
                  ...
                  //submit
                    if(isset($_POST['claim_remark']) && !empty($_POST['claim_remark'])  )
                    {  
                        $claim_remark=trim($_POST['claim_remark']);

                       
                            $url_is=the_api_authentication_api_url_is()."denkimAPILogic/MainPackages.AddClaimRemark";

                            $myvars='session_key='.$_SESSION['session_key'].'&_id='.$_id.'&claim_remark='.$claim_remark;

                             $header_array= array('Cookie:'.$_SESSION['cookie'],'Authorization:'.api_key_is(),'Origin:/repair_garage/claims_add_remark.php');
    ...
    <section class="content">
        <div class="container-fluid">
            ...
            <div class="row clearfix">
		     ...
		     <h4>Add remark to claim number: <?php echo $claim_number;?></h4>    
          <?php echo $message;?><br>
           
          <form action="<?php echo $full_link;?>" method="POST">
             <textarea name="claim_remark" cols="20" rows="10" placeholder="Remarks..."></textarea>

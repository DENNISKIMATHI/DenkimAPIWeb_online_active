...
if(isset($_POST['username']) && !empty($_POST['username']) && isset($_POST['password']) && !empty($_POST['password']))
{
	$username=  trim($_POST['username']);
        ...
        $url_is=the_api_authentication_api_url_is()."denkimAPILogic/MainPackages.AdministratorLogin";
        
        $myvars='username='.$username.'&password='.$password;
        
        $header_array= array('Authorization:'.api_key_is(),'Origin:/administrator/index.php');
        ...
        if($check_is==true)//if login is true
        {
            ...
            $_SESSION['cookie']=$returned_json_decoded["Set-Cookie"];
            $level=$returned_json_decoded["level"];
            
            
            switch ($level) 
            {
                    case 1:
                         header('location: ./senior_administrator');//
                    break;
                    
                    case 2:
                         header('location: ./junior_administrator');//
                    break;
                    
                    case 3:
                         header('location: ./claim_handlers');//
                    break;
                
                    case 4:
                         header('location: ./assessor_loss_adjsuter');//
                    break;
                    
                    case 5:
                         header('location: ./repair_garage');//
                    break;
                
                    case 6:
                         header('location: ./towing_rescue');//
                    break;
        ...
        else//else failed
        {
           ...
           header('location: ./?message=Wrong username and password combination&type=2');//
        }
}

...
{   
        ...
        $action_page='select_policy_user_optical_insurance.php?pn='.$policy_number.'&e='.$email_address.'&f='.$full_names.'&p='.$phone_number;
	 //fetch
        $returned_json_decoded= fetch_policy_type_specific($type_is,$policy_number,'/senior_administrator/select_policy_user_optical_insurance.php');
        ...
        if(isset($_POST['premium_options']) && !empty($_POST['premium_options']))
        {
            ...
            $how_many=count($_SESSION['shoping_cart'][$type_is]);//for motor
                                    ...
                                    $array_kart[$type_is][0]=array($item_id=>$items_array_is);
                                     ...
                                     check_out_kart_with_email($array_kart,$email_address,'/senior_administrator/select_policy_user_optical_insurance.php');


                                    header('location: view_user_optical_insurance.php?message=Policy selected&e='.$email_address.'&f='.$full_names.'&type=1&p='.$phone_number.'');//
        }
}

...
if(isset($_GET['pn']) && !empty($_GET['pn']) && !empty($_GET['pn']) && isset($_GET['e']) && !empty($_GET['e']) && isset($_GET['f']) && !empty($_GET['f'])  && isset($_GET['p']) && !empty($_GET['p']))
{   
        $type_is=2;//type
        ...
        $action_page='select_policy_user_in_patient_medical_insurance.php?pn='.$policy_number.'&e='.$email_address.'&f='.$full_names.'&p='.$phone_number;
	 //fetch
        $returned_json_decoded= fetch_policy_type_specific($type_is,$policy_number,'/senior_administrator/select_policy_user_in_patient_medical_insurance.php');
                           ...
                           // echo json_encode($childrens_array);
                            ...
                            if($any_truths>0)//assign if more than zero
                            {
                                ...
                                $array_kart[$type_is][0]=array($item_id=>$items_array_is);
                                 ...
                                 check_out_kart_with_email($array_kart,$email_address,'/senior_administrator/select_policy_user_in_patient_medical_insurance.php');


                                header('location: view_user_in_patient_medical_insurance.php?message=Policy selected&e='.$email_address.'&f='.$full_names.'&type=1&p='.$phone_number.'');//
                            }

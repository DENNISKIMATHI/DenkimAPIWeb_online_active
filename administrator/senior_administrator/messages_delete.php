...
if(  isset($_GET['re']) && !empty($_GET['re']) && isset($_GET['rn']) && !empty($_GET['rn']) )
{
	$recipient_email=trim($_GET['re']);
        $recipient_name=trim($_GET['rn']);
        
       
        $full_link="messages_delete.php?re=".$recipient_email."&rn=".$recipient_name;//for form submission
        $return_link="messages.php";
        
        $statement='Are you sure you want to delete all messages between '.strtoupper($recipient_name).' and you ? <span id="red_text_span"><a href="'.$full_link.'&a=yes">[YES]</a></span>  &nbsp;&nbsp;&nbsp;&nbsp;  <span id="green_text_span"><a href="'.$return_link.'">[NO]</a></span> ';
        ...
        if(isset($_GET['a']) && !empty($_GET['a']) )
        {
            ...
            {
                //delete
                            $url_is=the_api_authentication_api_url_is()."denkimAPILogic/MainPackages.FetchMessagesDelete";

                            $myvars='session_key='.$_SESSION['session_key'].'&recipient_email='.$recipient_email;

                             $header_array= array('Cookie:'.$_SESSION['cookie'],'Authorization:'.api_key_is(),'Origin:/senior_administrator/messages_delete.php');

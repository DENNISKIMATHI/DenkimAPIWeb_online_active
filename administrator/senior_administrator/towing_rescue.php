...
if(isset($_GET['c']) && !empty($_GET['c']) && isset($_GET['l']) && is_numeric($_GET['l']) && ( $_GET['s']==0 || is_numeric($_GET['s']) ) &&  isset($_GET['sc']) && !empty($_GET['sc']) && isset($_GET['so']) && !empty($_GET['so']) && isset($_GET['re']) && !empty($_GET['re']))
{
	$column=trim($_GET['c']);
        ...
        $full_link="towing_rescue.php?c=".$column."&l=".$limit."&s=".$skip."&sc=".$sort_column."&so=".$sort_order."&re=".$rows_every;//for form submission
        $link_without_sort_column_sort_order="towing_rescue.php?c=".$column."&l=".$limit."&s=".$skip."&re=".$rows_every;//for headers sorting
        $link_without_limit_skip_rows_every="towing_rescue.php?c=".$column."&sc=".$sort_column."&so=".$sort_order;//for browsing
        $delete_link="towing_rescue_delete.php?c=".$column."&l=".$limit."&s=".$skip."&sc=".$sort_column."&so=".$sort_order."&re=".$rows_every;//for form submission
        $rating_edit_link="towing_rescue_rating_edit.php?c=".$column."&l=".$limit."&s=".$skip."&sc=".$sort_column."&so=".$sort_order."&re=".$rows_every;//for form submission
        ...
        $header_array= array('Cookie:'.$_SESSION['cookie'],'Authorization:'.api_key_is(),'Origin:/senior_administrator/towing_rescue.php');
        ...
        else//else failed
        {
           ...
           // header('location: logs_view.php?c='.$_GET['c'].'&l='.$_GET['l'].'&s='.$_GET['s'].'&message='.$message_is.'&type=2');//
                    ...
                    {  
                        ...
                        {
                            $url_is=the_api_authentication_api_url_is()."denkimAPILogic/MainPackages.AdministratorCreateTowingAndRescue";

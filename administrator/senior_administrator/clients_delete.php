...
if(  isset($_GET['l']) && is_numeric($_GET['l']) && ( $_GET['s']==0 || is_numeric($_GET['s']) ) &&  isset($_GET['sc']) && !empty($_GET['sc']) && isset($_GET['so']) && !empty($_GET['so']) && isset($_GET['re']) && !empty($_GET['re']) && isset($_GET['e']) && !empty($_GET['e']) && isset($_GET['f']) && !empty($_GET['f']))
{
	$limit=trim($_GET['l']);
        ...
        $full_link="clients_delete.php?l=".$limit."&s=".$skip."&sc=".$sort_column."&so=".$sort_order."&re=".$rows_every."&e=".$email_address."&f=".$full_names;//for form submission
        ...
        if(isset($_GET['a']) && !empty($_GET['a']) )
        {
            ...
            {
                //delete
                            $url_is=the_api_authentication_api_url_is()."denkimAPILogic/MainPackages.AdministratorDeleteClient";

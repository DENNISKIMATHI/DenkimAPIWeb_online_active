...
{  
    ...
    $logo_url=trim($_POST['logo_url']);
        ...
        $url_is=the_api_authentication_api_url_is()."denkimAPILogic/MainPackages.PolicyCreateContractorsAllRiskInsurance";

        $myvars='session_key='.$_SESSION['session_key'].'&company_name='.$company_name.'&contract_price_multiplier='.$contract_price_multiplier.'&policy_number='.$policy_number.'&expiry_duration_days='.$expiry_duration_days.'&html_url='.$html_url.'&logo_url='.$logo_url;

        $header_array= array('Cookie:'.$_SESSION['cookie'],'Authorization:'.api_key_is(),'Origin:/senior_administrator/contractors_all_risk_insurance.php');
        ...
        if($check_is==true)//if check is true
        {
              ...
              header('location: contractors_all_risk_insurance.php?message='.$message_is.'&type=1');//
        }
        else//else failed
        {

            header('location: contractors_all_risk_insurance.php?message='.$message_is.'&type=2');//
        } 
...
//fetch
$returned_json_decoded= fetch_policy_type(4,'/senior_administrator/contractors_all_risk_insurance.php');
...
//get privacy items
$policy_privacy_array=fetch_policy_privacy('4','/senior_administrator/contractors_all_risk_insurance.php');
$policy_currency_array=fetch_policy_currency('4','/senior_administrator/motor_insurance.php');
                      ...
                      {//start of foreach $message_is as $value
                            ...
                            $table=$table.'<tr class="'.$row_color.'" id="row_data">
                                                          <td>'.($count+1).'</td>  
                                                                          ...
                                                                          <td id="privacy_td'.$count.'" >'.$is_private_status.' [<a href="policy_change_privacy.php?s=4&st='.$is_private_status.'&pn='.$policy_number.'&rp=contractors_all_risk_insurance.php" >'.$is_private_status_make.'</a>]</td>
                                                                          <td id="currency_td'.$count.'" ><select id="'.md5($policy_number).'" onchange="submit_me_as_currency(\'4\',\''.$policy_number.'\',\''.md5($policy_number).'\',\'contractors_all_risk_insurance.php\');">'.get_select_list_of_currency($policy_currency_array['message'],$policy_number,$currency_lists).'</select></td>
                                                                          <td id="delete_td'.$count.'" ><span id="red_text_span"><a href="policy_delete_type_specific.php?p='.$policy_number.'&t=4&s=contractors_all_risk_insurance.php" title="Remove '.$policy_number.'">Delete</a></span></td>   
                                                        </tr>';

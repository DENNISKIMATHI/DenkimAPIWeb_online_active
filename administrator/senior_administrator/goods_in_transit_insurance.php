...
isset($_POST['a_percentage']) && !empty($_POST['a_percentage']) &&
isset($_POST['b_percentage']) && !empty($_POST['b_percentage']) &&
...
{  
    ...
    $a_percentage=trim($_POST['a_percentage']);
    $b_percentage=trim($_POST['b_percentage']);
    ...
    $logo_url=trim($_POST['logo_url']);
        ...
        $url_is=the_api_authentication_api_url_is()."denkimAPILogic/MainPackages.PolicyCreateGoodInTransitInsurance";

        $myvars='session_key='.$_SESSION['session_key'].'&company_name='.$company_name.'&a_percentage='.$a_percentage.'&b_percentage='.$b_percentage.'&policy_number='.$policy_number.'&expiry_duration_days='.$expiry_duration_days.'&html_url='.$html_url.'&logo_url='.$logo_url;

        $header_array= array('Cookie:'.$_SESSION['cookie'],'Authorization:'.api_key_is(),'Origin:/senior_administrator/goods_in_transit_insurance.php');
        ...
        if($check_is==true)//if check is true
        {
              ...
              header('location: goods_in_transit_insurance.php?message='.$message_is.'&type=1');//
        }
        else//else failed
        {

            header('location: goods_in_transit_insurance.php?message='.$message_is.'&type=2');//
        } 
...
//fetch
$returned_json_decoded= fetch_policy_type(13,'/senior_administrator/goods_in_transit_insurance.php');
...
//get privacy items
$policy_privacy_array=fetch_policy_privacy('13','/senior_administrator/goods_in_transit_insurance.php');
$policy_currency_array=fetch_policy_currency('13','/senior_administrator/motor_insurance.php');
...
{ 
                     ...
                     $count=$skip;//make count skipped rows
                      ...
                      {//start of foreach $message_is as $value
                            ...
                            $a_percentage=$value['a_percentage'];
                            $b_percentage=$value['b_percentage'];
                            ...
                            $table=$table.'<tr class="'.$row_color.'" id="row_data">
                                                          <td>'.($count+1).'</td>  
                                                                          ...
                                                                          <td id="a_percentage_td'.$count.'" >'.$a_percentage.'</td>
                                                                          <td id="b_percentage_td'.$count.'" >'.$b_percentage.'</td>  
                                                                          ...
                                                                          <td id="privacy_td'.$count.'" >'.$is_private_status.' [<a href="policy_change_privacy.php?s=13&st='.$is_private_status.'&pn='.$policy_number.'&rp=goods_in_transit_insurance.php" >'.$is_private_status_make.'</a>]</td>
                                                                          <td id="currency_td'.$count.'" ><select id="'.md5($policy_number).'" onchange="submit_me_as_currency(\'13\',\''.$policy_number.'\',\''.md5($policy_number).'\',\'goods_in_transit_insurance.php\');">'.get_select_list_of_currency($policy_currency_array['message'],$policy_number,$currency_lists).'</select></td>
                                                                          <td id="delete_td'.$count.'" ><span id="red_text_span"><a href="policy_delete_type_specific.php?p='.$policy_number.'&t=13&s=goods_in_transit_insurance.php" title="Remove '.$policy_number.'">Delete</a></span></td>   
                                                        </tr>';
                      ...
                      $table_head='<tr bgcolor="white">
                                   <th>#</th>
                                      ...
                                      <th><a href="#"onmouseover="hover_link(\'a_percentage_td\',\''.$count.'\');" onmouseout="out_link(\'a_percentage_td\',\''.$count.'\');" >A percentage</a></th>
                                       <th><a href="#" onmouseover="hover_link(\'b_percentage_td\',\''.$count.'\');" onmouseout="out_link(\'b_percentage_td\',\''.$count.'\');" >B percentage</a></th>
    ...
    <section class="content">
        <div class="container-fluid">
          ...
          <form action="" method="POST">
            ...
            <input type="number" name="a_percentage" step=any placeholder="A percentage"/>
            <input type="number" name="b_percentage" step=any placeholder="B percentage"/>

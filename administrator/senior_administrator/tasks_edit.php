...
if( isset($_GET['l']) && is_numeric($_GET['l']) 
        ...
        isset($_GET['_id']) && !empty($_GET['_id']) &&
         ...
         isset($_GET['reminder_date_time']) && !empty($_GET['reminder_date_time']) &&
         isset($_GET['repeats']) && !empty($_GET['repeats']) 
        )
{
        ...
        $_id=trim($_GET['_id']);
        $task=trim($_GET['task']);
        $reminder_date_time=trim($_GET['reminder_date_time']);
        $repeats=trim($_GET['repeats']);
        
        
        $explode_date_time= explode(' ', $reminder_date_time);
        
            $explode_date=explode('-', $explode_date_time[1]);
            //$explode_time=explode(':', $explode_date_time[0]);
         
            $parsable_date=$explode_date[2].'-'.$explode_date[1].'-'.$explode_date[0];
            //$parsable_time=$explode_time[0].'-'.$explode_time[1];
        
        $return_link="tasks.php?l=".$limit."&s=".$skip;
        $full_link="tasks_edit.php?l=".$limit."&s=".$skip."&_id=".$_id."&task=".$task."&reminder_date_time=".$reminder_date_time."&repeats=".$repeats;//for headers sorting
        ...
        if(isset($_POST['task']) && !empty($_POST['task']) && 
                    ...
                    {  
                        ...
                        $repeats=trim($_POST['repeats']);
                            ...
                            $url_is=the_api_for_php_is_url_is()."edit_diary/";
    ...
    <section class="content">
        <div class="container-fluid">
		   ...
		   <?php echo $message;?><br>
                   ...
                   <textarea name="task" cols="20" rows="10" maxlength="" placeholder="Task" required><?php echo $task;?></textarea><br><br>
                   <input type="date"  name="date_is" required value="<?php echo $parsable_date;?>" /><br><br>
                   <input type="time" name="time_is" required value="<?php echo $explode_date_time[0];?>"/><br><br>
                   
                   <select name="repeats" required>
                       <option value="<?php echo $repeats;?>"><?php echo return_task_repeats_array()[$repeats];?></option>
                       <?php

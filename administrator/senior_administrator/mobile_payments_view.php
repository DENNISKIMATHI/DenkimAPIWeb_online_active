...
if( isset($_GET['l']) && is_numeric($_GET['l']) && 
        ...
        isset($_GET['seen_status_is']) && !empty($_GET['seen_status_is']) 
    )
{
        ...
        $_id=trim($_GET['_id']);  
        $mode_of_payment=trim($_GET['mode_of_payment']);
        $amount_paid=trim($_GET['amount_paid']);
        $particulars=trim($_GET['particulars']);
        $time_date_of_payment=trim($_GET['time_date_of_payment']);
        $transaction_code=trim($_GET['transaction_code']);
        $msidn=trim($_GET['msidn']);
        $seen_status_is=trim($_GET['seen_status_is']);
        
        $full_link="mobile_payments_view.php?l=".$limit."&s=".$skip."&sc=".$sort_column."&so=".$sort_order."&re=".$rows_every.'&_id='.$_id.'&mode_of_payment='.$mode_of_payment.'&amount_paid='.$amount_paid.'&particulars='.$particulars.'&time_date_of_payment='.$time_date_of_payment.'&transaction_code='.$transaction_code.'&msidn='.$msidn.'&seen_status_is='.$seen_status_is; ;//for form submission
        $return="mobile_payments.php?l=".$limit."&s=".$skip."&sc=".$sort_column."&so=".$sort_order."&re=".$rows_every;//for form submission
      
        if($seen_status_is=='unseen')
        {
            //make seen
          
            
                        //fetch
                    $url_is=the_api_authentication_api_url_is()."denkimAPILogic/MainPackages.AdministratorMakeMobilePaymentSeen";

                    $myvars='session_key='.$_SESSION['session_key'].'&_id='.$_id;

                    $header_array= array('Cookie:'.$_SESSION['cookie'],'Authorization:'.api_key_is(),'Origin:/senior_administrator/mobile_payments_view.php');
       ...
       $table='<table class="table table-bordered table-hover table-responsive">
                    <tr>
                        ...
                        <td>'.strtoupper($mode_of_payment).'</td>
                    </tr>
                    <tr>
                        <th>Amount paid</th>
                        <td>KES. '.number_format($amount_paid).'</td>
                    </tr>
                    <tr>
                        <th>Particulars</th>
                        <td>'.$particulars.'</td>
                    ...
                    <tr>
                        <th>Phone number</th>
                        <td>'.strtoupper($msidn).'</td>
                    </tr>
                    <tr>
                        <th>Seen status</th>
                        <td>'.strtoupper($seen_status_is).'</td>
                    </tr>
               </table>';
...
<head>
    ...
    <meta content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no" name="viewport">
         <title>View Mobile payments</title>
    <!-- Favicon-->

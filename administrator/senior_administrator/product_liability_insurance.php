...
isset($_POST['minimum']) && !empty($_POST['minimum']) &&
...
{  
    ...
    $logo_url=trim($_POST['logo_url']);
        ...
        $url_is=the_api_authentication_api_url_is()."denkimAPILogic/MainPackages.PolicyCreateProductLiabilityInsurance";

        $myvars='session_key='.$_SESSION['session_key'].'&company_name='.$company_name.'&premium_percentage='.$premium_percentage.'&minimum='.$minimum.'&policy_number='.$policy_number.'&expiry_duration_days='.$expiry_duration_days.'&html_url='.$html_url.'&logo_url='.$logo_url;

        $header_array= array('Cookie:'.$_SESSION['cookie'],'Authorization:'.api_key_is(),'Origin:/senior_administrator/product_liability_insurance.php');
        ...
        if($check_is==true)//if check is true
        {
              ...
              header('location: product_liability_insurance.php?message='.$message_is.'&type=1');//
        }
        else//else failed
        {

            header('location: product_liability_insurance.php?message='.$message_is.'&type=2');//
        } 
...
//fetch
$returned_json_decoded= fetch_policy_type(14,'/senior_administrator/product_liability_insurance.php');
...
//get privacy items
$policy_privacy_array=fetch_policy_privacy('14','/senior_administrator/product_liability_insurance.php');
$policy_currency_array=fetch_policy_currency('14','/senior_administrator/motor_insurance.php');
                      ...
                      {//start of foreach $message_is as $value
                            ...
                            $table=$table.'<tr class="'.$row_color.'" id="row_data">
                                                          <td>'.($count+1).'</td>  
                                                                          ...
                                                                          <td id="minimum_td'.$count.'" >'.number_format($minimum,2).'</td>  
                                                                          ...
                                                                          <td id="privacy_td'.$count.'" >'.$is_private_status.' [<a href="policy_change_privacy.php?s=14&st='.$is_private_status.'&pn='.$policy_number.'&rp=product_liability_insurance.php" >'.$is_private_status_make.'</a>]</td>
                                                                          <td id="currency_td'.$count.'" ><select id="'.md5($policy_number).'" onchange="submit_me_as_currency(\'14\',\''.$policy_number.'\',\''.md5($policy_number).'\',\'product_liability_insurance.php\');">'.get_select_list_of_currency($policy_currency_array['message'],$policy_number,$currency_lists).'</select></td>
                                                                          <td id="delete_td'.$count.'" ><span id="red_text_span"><a href="policy_delete_type_specific.php?p='.$policy_number.'&t=14&s=product_liability_insurance.php" title="Remove '.$policy_number.'">Delete</a></span></td>   
                                                        </tr>';
                      ...
                      $table_head='<tr bgcolor="white">
                                   <th>#</th>
                                      ...
                                      <th><a href="#"onmouseover="hover_link(\'premium_percentage_td\',\''.$count.'\');" onmouseout="out_link(\'premium_percentage_td\',\''.$count.'\');" >Premium percentage</a></th>
                                       <th><a href="#" onmouseover="hover_link(\'minimum_td\',\''.$count.'\');" onmouseout="out_link(\'minimum_td\',\''.$count.'\');" >Minimum</a></th>
    ...
    <section class="content">
        <div class="container-fluid">
          ...
          <form action="" method="POST">
            ...
            <input type="number" name="premium_percentage" required step=any placeholder="Premium percentage"/>

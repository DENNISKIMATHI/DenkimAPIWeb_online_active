...
if(isset($_GET['e']) && !empty($_GET['e']) && isset($_GET['f']) && !empty($_GET['f']) && isset($_GET['p']) && !empty($_GET['p']))
{
                ...
                $type=5;
                //fetch
            $returned_json_decoded= fetch_policy_user_type($type,$email_address,$_SESSION['session_key'],$_SESSION['cookie'],'/senior_administrator/view_user_performance_bond_insurance.php');
            ...
            if($check_is==true)//if check is true
            {
                ...
                foreach ($message_is as $value) 
                {
                    ...
                    $policy_info= fetch_policy_type_specific($type,$policy_number,'/senior_administrator/view_user_performance_bond_insurance.php');
                    ...
                    if($policy_infocheck_is==true)
                    {
                          //echo json_encode($policy_infomessage_is).'<hr>';
                            ...
                            $delete_link='policy_delete_user_type_specific.php?_id='.$policy_id.'&e='.$email_address.'&f='.$full_names.'&p='.$policy_number.'&s=view_user_performance_bond_insurance.php&phn='.$phone_number;
                            $full_delete_link='<span id="red_text_span"><a href="'.$delete_link.'" title="Delete '.$policy_number.' for '.$full_names.'">Delete</a></span>';
                            //activate
                            $activate_deactivate_link='policy_activate_deactivate.php?s=view_user_performance_bond_insurance.php&t='.$type.'&e='.$email_address.'&f='.$full_names.'&pi='.$returned_array['policy_id'].'&as='.$returned_array['status'].'&cn='.$returned_array['company_name'].'&p='.$phone_number.'&ex='.$returned_array['expiry_duration_days'];
                            ...
                            if($diff_time_is<5000)//less than 5 seconds
                            {
                                ...
                                $message_is_is=login_behalf_of_client($email_address,'/senior_administrator/view_user_performance_bond_insurance.php');
                                             
                                $message_send="Hello ".$full_names.", Performance bond policy to be insured by ".$returned_array['company_name']." has been selected. The policy will be activated after payment of KES. ". number_format($returned_array['total'])." is made. Payment to be done via Denkim Insurance Portal. Click the following link to log in and pay. ".$message_is_is;
...
<head>
    ...
    <meta content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no" name="viewport">
       <title>View user performance bond insurance</title>
    <!-- Favicon-->

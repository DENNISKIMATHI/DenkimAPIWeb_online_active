...
if(  isset($_GET['l']) && !empty($_GET['l'])  && ( $_GET['s']==0 || is_numeric($_GET['s']) )  && isset($_GET['re']) && !empty($_GET['re']) && isset($_GET['fn']) && !empty($_GET['fn']) && isset($_GET['_id']) && !empty($_GET['_id']) && isset($_GET['h']) && !empty($_GET['h']) )
{
	$limit=trim($_GET['l']);
        $skip=trim($_GET['s']);
        ...
        $full_link="messages_view_delete.php?l=".$limit."&s=".$skip."&re=".$recipient_email."&fn=".$recipient_name."&_id=".$_id."&h=".$header;//for form submission
        $return_link="messages_view.php?l=".$limit."&s=".$skip."&re=".$recipient_email."&fn=".$recipient_name;//for form submission
        
        $statement='Are you sure you want to delete '.strtoupper($header).' message? <span id="red_text_span"><a href="'.$full_link.'&a=yes">[YES]</a></span>  &nbsp;&nbsp;&nbsp;&nbsp;  <span id="green_text_span"><a href="'.$return_link.'">[NO]</a></span> ';
        ...
        if(isset($_GET['a']) && !empty($_GET['a']) )
        {
            ...
            {
                //delete
                            $url_is=the_api_authentication_api_url_is()."denkimAPILogic/MainPackages.FetchMessagesThreadDelete";

                            $myvars='session_key='.$_SESSION['session_key'].'&_id='.$_id;

                             $header_array= array('Cookie:'.$_SESSION['cookie'],'Authorization:'.api_key_is(),'Origin:/senior_administrator/messages_view_delete.php');
...
<head>
    ...
    <meta content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no" name="viewport">
      <title>Delete message thread</title>
    <!-- Favicon-->

...
if(isset($_GET['c']) && !empty($_GET['c']) && isset($_GET['l']) && is_numeric($_GET['l']) && ( $_GET['s']==0 || is_numeric($_GET['s']) ) &&  isset($_GET['sc']) &&  isset($_GET['cr']) && !empty($_GET['sc']) && isset($_GET['so']) && !empty($_GET['so']) && isset($_GET['re']) && !empty($_GET['re']) && isset($_GET['e']) && !empty($_GET['e']) && isset($_GET['f']) && !empty($_GET['f']) && !empty($_GET['cr']))
{
	$column=trim($_GET['c']);
        ...
        $full_link="repair_garage_rating_edit.php?c=".$column."&l=".$limit."&s=".$skip."&sc=".$sort_column."&so=".$sort_order."&re=".$rows_every."&e=".$email_address."&f=".$full_names."&cr=".$current_rating;//for form submission
        ...
        if(isset($_POST['nr']) && !empty($_POST['nr']) )
        {
            $new_rating=  strtolower(trim($_POST['nr']));
                ...
                //change rating
                            $url_is=the_api_authentication_api_url_is()."denkimAPILogic/MainPackages.AdministratorEditRatingRepairGarage";

                            $myvars='session_key='.$_SESSION['session_key'].'&email_address='.$email_address.'&rating='.$new_rating;

                             $header_array= array('Cookie:'.$_SESSION['cookie'],'Authorization:'.api_key_is(),'Origin:/senior_administrator/repair_garage_rating_edit.php');
...
<head>
    ...
    <meta content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no" name="viewport">
         <title>Edit repair garage rating</title>
    <!-- Favicon-->

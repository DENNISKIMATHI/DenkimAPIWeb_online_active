...
if(  isset($_GET['p']) && !empty($_GET['p']) && isset($_GET['t']) && !empty($_GET['t']) && isset($_GET['s']) && !empty($_GET['s']) )
{
	$policy_number=trim($_GET['p']);
        ...
        $full_link="policy_delete_type_specific.php?p=".$policy_number."&t=".$type."&s=".$source;//for form submission
        
        $statement='Are you sure you want to delete '.strtoupper($policy_number).'? <span id="red_text_span"><a href="'.$full_link.'&a=yes">[YES]</a></span>  &nbsp;&nbsp;&nbsp;&nbsp;  <span id="green_text_span"><a href="'.$source.'">[NO]</a></span> ';
        ...
        if(isset($_GET['a']) && !empty($_GET['a']) )
        {
            ...
            {
                //delete
                            $url_is=the_api_authentication_api_url_is()."denkimAPILogic/MainPackages.PolicyDeleteTypeSpecific";

                            $myvars='session_key='.$_SESSION['session_key'].'&policy_number='.$policy_number.'&type='.$type;

                             $header_array= array('Cookie:'.$_SESSION['cookie'],'Authorization:'.api_key_is(),'Origin:/senior_administrator/policy_delete_type_specific.php');
                            ...
                            if($check_is==true)//if check is true
                            {
                                  ...
                                  header('location: '.$source.'?message='.$message_is.'&type=1');//
                            }
                            else//else failed
                            {

                                header('location: '.$source.'?message='.$message_is.'&type=2');//
                            } 
            }
            else
            {
...
<head>
    ...
    <meta content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no" name="viewport">
          <title>Policy Delete Type Specific</title>
    <!-- Favicon-->

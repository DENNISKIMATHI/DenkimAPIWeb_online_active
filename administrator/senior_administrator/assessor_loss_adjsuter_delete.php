...
if(isset($_GET['c']) && !empty($_GET['c']) && isset($_GET['l']) && is_numeric($_GET['l']) && ( $_GET['s']==0 || is_numeric($_GET['s']) ) &&  isset($_GET['sc']) && !empty($_GET['sc']) && isset($_GET['so']) && !empty($_GET['so']) && isset($_GET['re']) && !empty($_GET['re']) && isset($_GET['e']) && !empty($_GET['e']) && isset($_GET['f']) && !empty($_GET['f']))
{
	$column=trim($_GET['c']);
        ...
        $full_link="assessor_loss_adjsuter_delete.php?c=".$column."&l=".$limit."&s=".$skip."&sc=".$sort_column."&so=".$sort_order."&re=".$rows_every."&e=".$email_address."&f=".$full_names;//for form submission
        ...
        if(isset($_GET['a']) && !empty($_GET['a']) )
        {
            ...
            {
                //delete
                            $url_is=the_api_authentication_api_url_is()."denkimAPILogic/MainPackages.AdministratorDeleteAssessorLossAdjuster";

                            $myvars='session_key='.$_SESSION['session_key'].'&email='.$email_address;

                             $header_array= array('Cookie:'.$_SESSION['cookie'],'Authorization:'.api_key_is(),'Origin:/senior_administrator/assessor_loss_adjsuter_delete.php');
...
<head>
    ...
    <meta content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no" name="viewport">
         <title>Delete Assessor/Loss adjuster</title>
    <!-- Favicon-->

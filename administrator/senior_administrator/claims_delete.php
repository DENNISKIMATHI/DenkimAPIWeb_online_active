...
if(isset($_GET['l']) && is_numeric($_GET['l']) && ( $_GET['s']==0 || is_numeric($_GET['s']) ) && isset($_GET['re']) && !empty($_GET['re']) && isset($_GET['e']) && !empty($_GET['e']) && isset($_GET['f']) && !empty($_GET['f'])  && isset($_GET['_id']) && !empty($_GET['_id']) && isset($_GET['cn']) && !empty($_GET['cn']))
{
        ...
        $_id=trim($_GET['_id']);
        $claim_number=trim($_GET['cn']);
        
        
        $full_link="claims_delete.php?l=".$limit."&s=".$skip."&re=".$rows_every."&e=".$email_address."&f=".$full_names."&_id=".$_id."&cn=".$claim_number;//for form submission
        $return_link="claims.php?l=".$limit."&s=".$skip."&re=".$rows_every."&e=".$email_address."&f=".$full_names;
        
        $statement='Are you sure you want to delete '.strtoupper($claim_number).'? <span id="red_text_span"><a href="'.$full_link.'&a=yes">[YES]</a></span>  &nbsp;&nbsp;&nbsp;&nbsp;  <span id="green_text_span"><a href="'.$return_link.'">[NO]</a></span> ';
        ...
        if(isset($_GET['a']) && !empty($_GET['a']) )
        {
            ...
            {
                //delete
                            $url_is=the_api_authentication_api_url_is()."denkimAPILogic/MainPackages.DeleteUserClaimsSpecific";

                            $myvars='session_key='.$_SESSION['session_key'].'&email_address='.$email_address.'&_id='.$_id;
...
<head>
 ...
 <title>Delete claim specific</title>
    <!-- Favicon-->

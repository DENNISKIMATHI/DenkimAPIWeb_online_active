...
isset($_POST['building_multiplier']) && !empty($_POST['building_multiplier']) &&
isset($_POST['content_multiplier']) && !empty($_POST['content_multiplier']) &&
isset($_POST['electronics_multiplier']) && !empty($_POST['electronics_multiplier']) &&
...
{  
    ...
    $building_multiplier=trim($_POST['building_multiplier']);
    $content_multiplier=trim($_POST['content_multiplier']);
    $electronics_multiplier=trim($_POST['electronics_multiplier']);
    ...
    $logo_url=trim($_POST['logo_url']);
        ...
        $url_is=the_api_authentication_api_url_is()."denkimAPILogic/MainPackages.PolicyCreateHomeInsurance";

        $myvars='session_key='.$_SESSION['session_key'].'&company_name='.$company_name.'&building_multiplier='.$building_multiplier.'&content_multiplier='.$content_multiplier.'&electronics_multiplier='.$electronics_multiplier.'&policy_number='.$policy_number.'&expiry_duration_days='.$expiry_duration_days.'&html_url='.$html_url.'&logo_url='.$logo_url;

        $header_array= array('Cookie:'.$_SESSION['cookie'],'Authorization:'.api_key_is(),'Origin:/senior_administrator/home_insurance.php');
        ...
        if($check_is==true)//if check is true
        {
              ...
              header('location: home_insurance.php?message='.$message_is.'&type=1');//
        }
        else//else failed
        {

            header('location: home_insurance.php?message='.$message_is.'&type=2');//
        } 
...
//fetch
$returned_json_decoded= fetch_policy_type(7,'/senior_administrator/home_insurance.php');
...
if($check_is==true)//if check is true
{
                     ...
                     $count=$skip;//make count skipped rows
                      ...
                      {//start of foreach $message_is as $value
                            ...
                            $building_multiplier=$value['building_multiplier'];
                            $content_multiplier=$value['content_multiplier'];
                            $electronics_multiplier=$value['electronics_multiplier'];
                            ...
                            $table=$table.'<tr class="'.$row_color.'" id="row_data">
                                                          <td>'.($count+1).'</td>  
                                                                          ...
                                                                          <td id="building_multiplier_td'.$count.'" >'.$building_multiplier.'</td>
                                                                          <td id="content_multiplier_td'.$count.'" >'.$content_multiplier.'</td>  
                                                                          <td id="electronics_multiplier_td'.$count.'" >'.$electronics_multiplier.'</td> 
                      ...
                      $table_head='<tr bgcolor="white">
                                   <th>#</th>
                                      ...
                                      <th><a href="#"onmouseover="hover_link(\'building_multiplier_td\',\''.$count.'\');" onmouseout="out_link(\'building_multiplier_td\',\''.$count.'\');" >Building multiplier</a></th>
                                       <th><a href="#" onmouseover="hover_link(\'content_multiplier_td\',\''.$count.'\');" onmouseout="out_link(\'content_multiplier_td\',\''.$count.'\');" >Content multiplier</a></th>
                                      <th><a href="#" onmouseover="hover_link(\'electronics_multiplier_td\',\''.$count.'\');" onmouseout="out_link(\'electronics_multiplier_td\',\''.$count.'\');" >Electronics multiplier</a></th>
    ...
    <section class="content">
        <div class="container-fluid">
          ...
          <form action="" method="POST">
            ...
            <input type="text" name="building_multiplier" placeholder="Building multiplier"/>
            <input type="text" name="content_multiplier"  placeholder="Content multiplier"/>
            <input type="text" name="electronics_multiplier"  placeholder="Electronics multiplier"/>

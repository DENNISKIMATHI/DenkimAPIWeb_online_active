...
{   
        ...
        //fetch email
        $personal_details_array=fetch_personal_details(3,$_SESSION['session_key'],$_SESSION['cookie'],'/client/console/select_policy_user_maternity_insurance.php');
        $email_address=$personal_details_array['email_address'];
        
        $policy_number=trim($_GET['pn']);
        
        $action_page='select_policy_user_maternity_insurance.php?pn='.$policy_number;
	 //fetch
        $returned_json_decoded= fetch_policy_type_specific($type_is,$policy_number,'/client/console/select_policy_user_maternity_insurance.php');
        ...
        if($check_is==true)//if check is true
        {
            ...
            $radio_buttons=  make_maternity_radio_buttons($options,'premium_options');
        }
        else
        {
                    if($message_is=='')
        ...
        if(isset($_POST['premium_options']) && !empty($_POST['premium_options']))
        {
            ...
            $how_many=count($_SESSION['shoping_cart'][$type_is]);//for motor
                                    ...
                                    $details_are=fetch_personal_details(3,$_SESSION['session_key'],$_SESSION['cookie'],'/shopping_kart/checkout_login.php');
                                     check_out_kart_with_email($array_kart,$email_address,'/client/console/select_policy_user_maternity_insurance.php',$details_are['full_names']);


                                    header('location: view_user_maternity_insurance.php?message=Policy selected&type=1');//
        }
}
...
<head>
    ...
    <meta content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no" name="viewport">
         <title>Maternity insurance select</title>
    <!-- Favicon-->

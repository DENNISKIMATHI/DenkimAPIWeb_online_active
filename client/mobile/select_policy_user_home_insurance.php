...
{   
        ...
        //fetch email
        $personal_details_array=fetch_personal_details(3,$_SESSION['session_key'],$_SESSION['cookie'],'/client/console/select_policy_user_home_insurance.php');
        $email_address=$personal_details_array['email_address'];
        
        $policy_number=trim($_GET['pn']);
        
        $action_page='select_policy_user_home_insurance.php?pn='.$policy_number;
	 //fetch
        $returned_json_decoded= fetch_policy_type_specific($type_is,$policy_number,'/client/console/select_policy_user_home_insurance.php');
        ...
        if($check_is==true)//if check is true
        {
            ...
            $building_multiplier=$message_is['building_multiplier'];
            $content_multiplier=$message_is['content_multiplier'];
            $electronics_multiplier=$message_is['electronics_multiplier'];
            ...
            $html_url_content=  send_curl_post($html_url, null, null);
        ...
        else
        {
         ...
         //submit
        if( (isset($_POST['building_value']) && !empty($_POST['building_value']) ) || (isset($_POST['content_value']) && !empty($_POST['content_value']) ) || (isset($_POST['electronics_value']) && !empty($_POST['electronics_value']) ) )
        {
            $building_value=trim($_POST['building_value']);
            $content_value=trim($_POST['content_value']);
            $electronics_value=trim($_POST['electronics_value']);
            
            //resolve empty
            $building_value=$building_value==""? 0 : $building_value;
            $content_value=$content_value==""? 0 : $content_value;
            $electronics_value=$electronics_value==""? 0 : $electronics_value;
            ...
            $items_array_is=array('policy_number'=>$policy_number,
                                ...
                                'electronics_value'=>$electronics_value,
                                'building_multiplier'=>$building_multiplier,
                                'content_multiplier'=>$content_multiplier,
                                'electronics_multiplier'=>$electronics_multiplier,
            ...
            $details_are=fetch_personal_details(3,$_SESSION['session_key'],$_SESSION['cookie'],'/shopping_kart/checkout_login.php');
             check_out_kart_with_email($array_kart,$email_address,'/client/console/select_policy_user_home_insurance.php',$details_are['full_names']);
            
           
            header('location: view_user_home_insurance.php?message=Policy selected&type=1');//
        }
...
<head>
    ...
    <meta content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no" name="viewport">
	 <title>Home insurance select</title>
    <!-- Favicon-->
    ...
    <section class="content">
        <div class="container-fluid">
		  ...
		  <div class="row clearfix">
            ...
            <h4>1.) Enter total value of the building,Perimeter wall and gate (NON RENTAL)</h4>
            <input type="number" class="form-control" min="1" name="building_value" building_value_multiplier_is="<?php echo $building_multiplier;?>" id="building_value" placeholder="Total value of building, perimeter wall and gate( excluding land value and content)"/>
            
          <h4> Premium   <span id="building_value_span"></span></h4>
            <br>
         
            
            
            <h4>2.) Enter total value of furniture, fitting, clothes,shoes,beddings.</h4>
            <input type="number" class="form-control" min="1" name="content_value" content_value_multiplier_is="<?php echo $content_multiplier;?>" id="content_value" placeholder="Total Value of content e.g. furniture, clothing ( excluding electronics)"/>
            
            
            <h4>Premium <span id="content_value_span"></span></h4>
            <br>
            
			
            <h4>3.) Enter total value of electronics and portable devices </h4>
            <input type="number" class="form-control" min="1" name="electronics_value" electronics_value_multiplier_is="<?php echo $electronics_multiplier;?>" id="electronics_value" placeholder="Total Value of electronics, portable devices e.g. phones, jewellery, cameras.)"/>
          
			<h4>Premium  <span id="electronics_value_span"></span> </h4>
        ...
        <script type="text/javascript" src="../../javascript/home_insurance_select.js"></script>

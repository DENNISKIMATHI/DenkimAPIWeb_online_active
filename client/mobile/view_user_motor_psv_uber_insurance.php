...
if(isset($_GET['message']) && !empty($_GET['message']) && isset($_GET['type']) && !empty($_GET['type']))
{
	$message=$_GET['message'];
	 ...
	 $message='<span id="'.$good_bad_id.'">'.$message.'</span>';
                ...
                //fetch email
        $personal_details_array=fetch_personal_details(3,$_SESSION['session_key'],$_SESSION['cookie'],'/client/console/view_user_motor_psv_uber_insurance.php');
        $email_address=$personal_details_array['email_address'];
        
                $type=15;
                //fetch
            $returned_json_decoded= fetch_policy_user_type($type,$email_address,$_SESSION['session_key'],$_SESSION['cookie'],'/client/console/view_user_motor_psv_uber_insurance.php');
            ...
            if($check_is==true)//if check is true
            {
                ...
                foreach ($message_is as $value) 
                {
                   //echo json_encode($value).'<hr>';
                    ...
                    $policy_info= fetch_policy_type_specific($type,$policy_number,'/client/console/view_user_motor_psv_uber_insurance.php');
                    ...
                    if($policy_infocheck_is==true)
                    {
                         //echo json_encode($policy_infomessage_is).'<hr>';
                            ...
                            $payments_link='policy_view_payments_specific.php?s=view_user_motor_psv_uber_insurance.php&t='.$type.'&pi='.$returned_array['policy_id'].'&pn='.$returned_array['policy_number'].'&cn='.$returned_array['company_name'].'&pd='.$returned_array['policy_date'].'&edd='.$returned_array['expiry_duration_days'].'&pt='.$returned_array['total'];
                            $make_payments_link='policy_make_payments_specific.php?s=view_user_motor_psv_uber_insurance.php&t='.$type.'&pi='.$returned_array['policy_id'].'&pn='.$returned_array['policy_number'].'&cn='.$returned_array['company_name'].'&pd='.$returned_array['policy_date'].'&edd='.$returned_array['expiry_duration_days'].'&pt='.$returned_array['total'];
...
<head>
    ...
    <meta content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no" name="viewport">
          <title>View Motor psv, uber, taxify, little cab and institutional buses and vans insurance</title>
    <!-- Favicon-->

...
{   
        $type_is=12;//type
         //fetch email
        $personal_details_array=fetch_personal_details(3,$_SESSION['session_key'],$_SESSION['cookie'],'/client/console/select_policy_user_public_liability_insurance.php');
        $email_address=$personal_details_array['email_address'];
        
        $policy_number=trim($_GET['pn']);
        
        $action_page='select_policy_user_public_liability_insurance.php?pn='.$policy_number;
	 //fetch
        $returned_json_decoded= fetch_policy_type_specific($type_is,$policy_number,'/client/console/select_policy_user_public_liability_insurance.php');
        ...
        if(isset($_POST['limit']) && !empty($_POST['limit']) )
        {
           ...
           $array_kart=array();
            $array_kart[$type_is][0]=array($item_id=>$items_array_is);
             ...
             check_out_kart_with_email($array_kart,$email_address,'/client/console/select_policy_user_public_liability_insurance.php',$personal_details_array['full_names']);
            
           
            header('location: view_user_public_liability_insurance.php?message=Policy selected&type=1');//
        ...
        <div class="container-fluid">
		  ...
		  <div class="row clearfix">
			  ...
			  <form action="<?php echo $action_page;?>" method="post">
               <h4>Enter the limit.(Minimum is: <?php echo number_format($minimum,2);?>)</h4>
            <input type="number" class="form-control" min="<?php echo $minimum;?>" required name="limit" premium_percentage_is="<?php echo $premium_percentage;?>" id="limit" placeholder="Limits of cover."/>
          <br>
